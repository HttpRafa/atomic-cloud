    ...
    pub async fn show(
        ...
        match connection.client.get_deployments().await {
            Ok(deployments) => {
                ...
                progress.end();
                match Select::new("From what deployment do want more information?", deployments)
                    .prompt()
                {
                    Ok(deployment) => {
                        let progress = Loading::default();
                        ...
                        match connection.client.get_deployment(&deployment).await {
                            Ok(deployment) => {
                                ...
                                if let Some(scaling) = deployment.scaling {
                                    ...
                                    info!("         <green><b>Max Players per unit</>: {}", scaling.max_players);
                                    info!("         <green><b>Start Threshold</>: {}%", scaling.start_threshold * 100.0);
                                    info!("         <green><b>Stop Empty Units</>: {}", scaling.stop_empty_units);
                                } else {
