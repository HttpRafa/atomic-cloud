...
impl ManageService for ManageServiceImpl {
    ...
    async fn request_stop(&self, request: Request<()>) -> Result<Response<()>, Status> {
        Ok(Response::new(
            TonicTask::execute_authorized::<(), _, _>(
                AuthType::User,
                Permissions::REQUEST_STOP,
                &self.0,
                request,
                |_, _| Ok(Box::new(RequestStopTask())),
            )
            .await?,
        ))
    }
    ...
    async fn get_plugins(
        ...
        Ok(Response::new(
            TonicTask::execute_authorized::<manage::plugin::List, _, _>(
                AuthType::User,
                Permissions::LIST,
                &self.0,
                request,
        ...
        Ok(Response::new(
            TonicTask::execute_authorized::<(), _, _>(
                ...
                |request, _| {
                    ...
                    let controller = request.controller_address.parse().map_err(|_| {
    ...
    async fn update_node(
        ...
        Ok(Response::new(
            TonicTask::execute_authorized::<manage::node::Detail, _, _>(
                AuthType::User,
    ...
    async fn get_nodes(
        ...
        Ok(Response::new(
            TonicTask::execute_authorized::<manage::node::List, _, _>(
                AuthType::User,
                Permissions::LIST,
                &self.0,
                request,
    ...
    async fn update_group(
        ...
        Ok(Response::new(
            TonicTask::execute_authorized::<manage::group::Detail, _, _>(
                AuthType::User,
    ...
    async fn get_groups(
        ...
        Ok(Response::new(
            TonicTask::execute_authorized::<common_group::List, _, _>(
                AuthType::User,
                Permissions::LIST,
                &self.0,
                request,
    ...
    async fn schedule_server(
        ...
        Ok(Response::new(
            TonicTask::execute_authorized::<String, _, _>(
                AuthType::User,
                Permissions::SCHEDULE_SERVER,
                &self.0,
                request,
                |request, _| {
                    let request = request.into_inner();
    ...
    async fn get_server(
        ...
        Ok(Response::new(
            TonicTask::execute_authorized::<manage::server::Detail, _, _>(
                AuthType::User,
                Permissions::GET_SERVER,
                &self.0,
                request,
                |request, _| {
                    let request = request.into_inner();
    ...
    async fn get_servers(
        ...
        Ok(Response::new(
            TonicTask::execute_authorized::<common_server::List, _, _>(
                AuthType::User,
                Permissions::LIST,
                &self.0,
                request,
    ...
    async fn write_to_screen(&self, request: Request<WriteReq>) -> Result<Response<()>, Status> {
        let auth = TonicTask::get_auth(AuthType::User, &request)?;
        if !auth.is_allowed(Permissions::WRITE_TO_SCREEN) {
            return Err(Status::permission_denied(INSUFFICIENT_PERMISSIONS_MESSAGE));
        }
    ...
    async fn subscribe_to_screen(
        ...
        let auth = TonicTask::get_auth(AuthType::User, &request)?;
        if !auth.is_allowed(Permissions::READ_SCREEN) {
            return Err(Status::permission_denied(INSUFFICIENT_PERMISSIONS_MESSAGE));
        }
    ...
    async fn get_user(
        ...
        Ok(Response::new(
            TonicTask::execute_authorized::<common_user::Item, _, _>(
                AuthType::User,
    ...
    async fn get_users(&self, request: Request<()>) -> Result<Response<common_user::List>, Status> {
        Ok(Response::new(
            TonicTask::execute_authorized::<common_user::List, _, _>(
                AuthType::User,
                Permissions::LIST,
                &self.0,
                request,
    ...
    async fn get_user_count(&self, request: Request<()>) -> Result<Response<u32>, Status> {
        Ok(Response::new(
            TonicTask::execute_authorized::<u32, _, _>(
                AuthType::User,
                Permissions::LIST,
                &self.0,
                request,
                |_, _| Ok(Box::new(UserCountTask)),
            )
            .await?,
        ))
    }
    ...
    async fn subscribe_to_power_events(
        ...
        let auth = TonicTask::get_auth(AuthType::User, &request)?;
        if !auth.is_allowed(Permissions::READ_POWER_EVENTS) {
            return Err(Status::permission_denied(INSUFFICIENT_PERMISSIONS_MESSAGE));
        }
    ...
    async fn subscribe_to_ready_events(
        ...
        let auth = TonicTask::get_auth(AuthType::User, &request)?;
        if !auth.is_allowed(Permissions::READ_READY_EVENTS) {
            return Err(Status::permission_denied(INSUFFICIENT_PERMISSIONS_MESSAGE));
        }
